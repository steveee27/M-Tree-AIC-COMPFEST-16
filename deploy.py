...
def recommend_job(user_input, df, vectorizer, tfidf_matrix, experience_levels, work_types, name):
    ...
    threshold = np.percentile(cosine_similarities, 95)
...
def load_job_data():
    ...
    url = 'https://drive.google.com/uc?id=1ardWFjmnCjWHa36mj_CacK5CuPHrJLId'
    output = 'linkedin.csv'
    gdown.download(url, output, quiet=False)

    df_job = pd.read_csv(output)
    df_job = df_job.drop_duplicates(subset=['job_posting_url'])
    df_job['Combined'] = df_job['name'] + ' ' + df_job['title'].fillna('') + ' ' + df_job['description_x'].fillna('') + ' ' + df_job['skills_desc'].fillna('')
...
def load_course_data():
    ...
    df_course = df_course.drop_duplicates(subset=['Title', 'URL', 'Short Intro'])
    translations = {
...
if page == '🏢 Home':
    ...
    st.title("💼 Job Insight System")

    # Job search filters
    st.markdown("<h3 style='font-size: 24px;'>🎚️ Experience Level</h3>", unsafe_allow_html=True)
    ...
    st.markdown("<h3 style='font-size: 24px;'>🏢 Work Type</h3>", unsafe_allow_html=True)
    ...
    name = st.text_input("🔍 Search Company Name")

    user_input = st.text_input("🧑‍💼 Tell me about your career profile (e.g., education background, key skills, project experience, certifications, and interests)")
    ...
    selected_subtitle = st.selectbox('🗣️ Select Subtitle Language', ['All'] + unique_subtitles)

    # User input for course preference
    user_input = st.text_input("🔍 Enter skills or topics you'd like to learn:")
